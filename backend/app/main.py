...
async def run_daily_cron_summarizer():
    ...
    while True:
        try:
            ...
            logger.info(
                "⏰ Running automated end-of-day interaction summarizer cron job..."
            )
            async with AsyncSessionLocal() as db:
                res = await db.execute(
                    select(User.id).where(User.role == "patient")
                )
                ...
                for pid in patient_ids:
                    ...
                    await auto_summarize_daily_interactions(
                        req=DummyReq(), db=db
                    )
            ...
            logger.error(
                f"Error in automated daily summarizer cron: {str(e)}"
            )
...
async def serve_patient_app():
    ...
    return JSONResponse(
        {
            "status": "online",
            "message": "RecoverAI API running. Patient UI loading.",
        }
    )
...
async def serve_caregiver_portal():
    ...
    if os.path.exists(file_path):
        return FileResponse(file_path)
    return JSONResponse(
        {"status": "online", "message": "Caregiver Portal loading."}
    )
