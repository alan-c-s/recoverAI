...
async def migrate_sqlite_columns():
    ...
    async with engine.begin() as conn:
        try:
            await conn.execute(text("ALTER TABLE recovery_checkins ADD COLUMN sentiment_label VARCHAR(50) DEFAULT 'Neutral'"))
            print("Added column sentiment_label to recovery_checkins")
        except Exception as e:
            print("Column sentiment_label note:", e)

        try:
            await conn.execute(text("ALTER TABLE recovery_checkins ADD COLUMN sentiment_score FLOAT DEFAULT 0.0"))
