...
class User(Base):
    ...
    risk_alerts = relationship(
        "RiskAlert",
        foreign_keys="RiskAlert.patient_id",
        back_populates="patient",
        cascade="all, delete-orphan",
    )
...
class MemoryEmbedding(Base):
    ...
    )  # 'trigger', 'coping_strategy', 'milestone', 'reflection'
    content = Column(Text, nullable=False)
    embedding_json = Column(
        Text, nullable=False
    )  # Stored as JSON string vector
...
class RiskAlert(Base):
    ...
    checkin_id = Column(
        String(36),
        ForeignKey("recovery_checkins.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    patient = relationship(
        "User", foreign_keys=[patient_id], back_populates="risk_alerts"
    )
