...
from app.schemas.schemas import PatientProfileUploadRequest
from app.services.rag_service import generate_embedding

router = APIRouter(
    prefix="/caregiver", tags=["Caregiver Portal & Patient Management"]
)
...
async def upload_patient_profile(
    req: PatientProfileUploadRequest, db: AsyncSession = Depends(get_db)
):
    ...
    res_map = await db.execute(
        select(PatientCaregiverMap).where(
            PatientCaregiverMap.patient_id == patient_id
        )
    )
    ...
    for item in grounding_entries:
        ...
        mem_obj = MemoryEmbedding(
            ...
            metadata_json=json.dumps(
                {"tags": item["tags"], "uploaded_by": "caregiver"}
            ),
        )
        db.add(mem_obj)
...
async def acknowledge_demo_alert(
